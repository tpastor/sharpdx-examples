...
namespace SharpExamples
{
    public class FreeLook
    {
        ...
        public FreeLook(Input input,float aspect)
        {
            Projection = Matrix.PerspectiveFovLH((float)Math.PI / 4, aspect, 1, 3000);
        ...
        public bool Update(float frameDelta)
        {
            ...
            if (Input.KeysDown.Count != 0)
            {
                ...
                float flySpeed = Input.KeysDown.Contains(Keys.ShiftKey) ? 50 : 15;
                ...
                {
                    translation += flySpeed * GetSizewaysTranslation(relDirection, -Math.PI / 2);
                }
